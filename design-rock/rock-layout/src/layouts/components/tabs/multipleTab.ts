...
import { PAGE_NOT_FOUND_NAME, PageEnum, REDIRECT_NAME } from '@rchitect-design/constants'
import type {
  RouteLocationNormalized,
  RouteLocationRaw,
  Router,
} from '@rchitect-rock/router'
import { getRawRoute } from '@rchitect-rock/tools'
import { THROWN_HANDLER, diKT, diK } from '@rchitect-rock/ioc'
import { Lib as routeLib } from '@rchitect-rock/router'
import { defineStore } from '@rchitect-rock/state'
import { MultipleTab } from '#/layouts/types'
...
function useGo(_router?: Router) {
  const { push, replace } = _router || diKT(routeLib.types.RouteTable).router
...
function handleError(e: Error) {
  diK(THROWN_HANDLER)?.error('logger', { message: e.message }, e)
}
...
export const useMultipleTab = defineStore({
  ...
  state: (): MultipleTab.MultipleTabState => ({
