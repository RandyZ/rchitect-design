...
<template>
  <WmqSpace vertical>
    <SwitchItem :title="t('layout.setting.splitMenu')" :def="getSplit" :event="HandlerSettingEnum.MENU_SPLIT"
    ...
    <SwitchItem :title="t('layout.setting.closeMixSidebarOnChange')" :def="getCloseMixSidebarOnChange"
      :event="HandlerSettingEnum.MENU_CLOSE_MIX_SIDEBAR_ON_CHANGE" :disabled="!getIsMixSidebar" />
    <SwitchItem :title="t('layout.setting.menuCollapse')" :def="getCollapsed" :event="HandlerSettingEnum.MENU_COLLAPSED"
      :disabled="!getShowMenuRef" />
    <SwitchItem :title="t('layout.setting.menuDrag')" :def="getCanDrag" :event="HandlerSettingEnum.MENU_HAS_DRAG"
      :disabled="!getShowMenuRef" />
    <SwitchItem :title="t('layout.setting.menuSearch')" :def="getShowSearch" :event="HandlerSettingEnum.HEADER_SEARCH"
      :disabled="!getShowHeader" />
    <SwitchItem :title="t('layout.setting.menuAccordion')" :def="getAccordion" :event="HandlerSettingEnum.MENU_ACCORDION"
      :disabled="!getShowMenuRef" />
    ...
    <SwitchItem :title="t('layout.setting.fixedHeader')" :def="getHeaderFixed" :event="HandlerSettingEnum.HEADER_FIXED"
      :disabled="!getShowHeader" />
    <SwitchItem :title="t('layout.setting.fixedSideBar')" :def="getMenuFixed" :event="HandlerSettingEnum.MENU_FIXED"
    ...
    <SelectItem :title="t('layout.setting.topMenuLayout')" :options="topMenuAlignOptions" :def="getTopMenuAlign"
      :event="HandlerSettingEnum.MENU_TOP_ALIGN"
      :disabled="!getShowHeader || getSplit || (!getIsTopMenu && !getSplit) || getIsMixSidebar" />
    <SelectItem :title="t('layout.setting.menuCollapseButton')" :options="triggerOptions" :def="triggerDef"
      :event="HandlerSettingEnum.MENU_TRIGGER" :disabled="!getShowMenuRef || getIsMixSidebar" />
    <SelectItem :title="t('layout.setting.contentMode')" :options="contentModeOptions" :def="getContentMode"
    ...
    <InputNumberItem :title="t('layout.setting.expandedMenuWidth')" :min="100" :max="600" :step="10" :def="getMenuWidth"
      suffix="px" :event="HandlerSettingEnum.MENU_WIDTH" :disabled="getShowMenuRef" />
  </WmqSpace>
</template>
