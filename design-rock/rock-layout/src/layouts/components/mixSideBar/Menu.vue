...
import { useGo, useMenuSetting } from "@rchitect-rock/hooks";
...
<template>
  <div :class="bem()" v-bind="getMenuEvents">
    <logo :class="[bem('logo'), 'shadow']" :style="{ '--un-shadow-color': 'var(--n-border-color)' }"
      v-if="getMenuType === MenuTypeEnum.MIX_SIDEBAR" :showTitle="false" />
    <WmqScrollbar :class="bem('scrollbar')">
      <ul :class="bem('module')">
        <li :class="[
          ...
          <WmqIconify :class="bem('module__icon')" :size="getCollapsed ? 16 : 20"
    ...
    <div :class="['shadow', bem('menu-list')]" :style="getMenuStyle" ref="sideRef">
      <div v-show="openMenu" :class="[
        ...
        <WmqIconify :size="16" :icon="getMixSideFixed ? 'ri:pushpin-2-fill' : 'ri:pushpin-2-line'" class="pushpin"
          @click="handleFixedMenu" hoverPointer />
      </div>
      <WmqH5 v-if="openMenu" :class="bem('menu-list__children-title')">{{ childrenTitle }}</WmqH5>
