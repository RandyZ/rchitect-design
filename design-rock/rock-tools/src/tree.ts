...
// tree from list
export function listToTree<T extends object>(list: T[], config?: TreeActionConfig): T[] {
  ...
  for (const node of list) {
    set(node, children, get(node, children, []))
    nodeMap.set(get(node, id), node)
  }
  for (const node of list) {
    const parent = nodeMap.get(get(node, pid))
...
export function findTreeNode<T = any>(
  ...
  for (const node of list) {
    ...
    const childrenArr = get(node, children, []) as T[];
    childrenArr && list.push(...childrenArr)
  }
  return null
}
