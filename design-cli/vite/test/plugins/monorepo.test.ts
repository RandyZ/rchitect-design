...
describe('【测试MonoRepoResolverPlugin】', () => {
  ...
  describe('测试customResolver解析组件路径', () => {
    test(`正常解析testpkg/src/index的形式`, async () => {
      const [, importerId] = buildTestPkgPath()
      expect(customResolver.call(null, 'src/index', importerId, null)).contains('test/plugins/testpkg/src/index.ts')
    })

    test(`正常解析testpkg/src/index.ts的形式`, async () => {
      const [, importerId] = buildTestPkgPath()
      expect(customResolver.call(null, 'src/index.ts', importerId, null)).contains('test/plugins/testpkg/src/index.ts')
    })
    ...
    // FIXME 这两个单元测试在jenkins中暂时无法通过，后续调查问题
  ...
  describe('测试包内互相引用', () => {
    test('测试src/index.ts中解析vue-ts的index', () => {
      const [demoPkgPath] = buildTestPkgPath()
      const importerId = path.resolve(demoPkgPath, './src/index.ts')
      expect(customResolver.call(null, 'src/vue-ts', importerId)).contains('testpkg/src/vue-ts/index.ts')
    })
  })
});
