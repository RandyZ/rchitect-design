...
import { getRequestToken, requestParams, resultError, resultSuccess } from "@rchitect-rock/tools/mock-util";
...
const Mocks:MockMethod[] = [
  ...
  {
    url: '/basic-api/v1/account/login/username',
    timeout: 200,
    method: 'post',
    response: ({ body }) => {
  ...
  {
    url: '/basic-api/v1/account/me/info',
    method: 'get',
    response: (request) => {
      const accessToken = getRequestToken(request)
      console.info('Randy~~~', request, accessToken)
      return {
        "code": HttpStatusCode.OK,
        "reason": "成功",
        "debugMsg": "成功",
