...
import type { NamePath } from "../types/namePath";
...
export function useFormEvents({
      ...
      if (hasKey || !!constructValue) {
        ...
        nestKeyArray.forEach((nestKey: string) => {
          try {
            ...
            if (isCustomDef(value)) {
              unref(formModel)[nestKey] = unref(value);
              validKeys.push(nestKey);
            }
          } catch (e) {
            // key not exist
            if (isCustomDef(defaultValueRef.value[nestKey])) {
  ...
  async function resetSchema(data: Partial<FormSchema> | Partial<FormSchema>[]) {
    ...
    if (!hasField) {
      diK(THROWN_HANDLER)
        ?.error('logger', { message: 'All children of the form Schema array that need to be updated must contain the `field` field' });
      return;
    }
