<script lang="tsx">
  ...
  export default defineComponent({
    ...
    setup(props, { slots }) {
      const driverHook = inject(DriverHookKey);
      const { t } = useI18n();
      ...
      function handleRules(): Form.Rules[] {
        const {
          rules: defRules = [],
          component,
        ...
        if (isFunction(dynamicRules)) {
          return dynamicRules(unref(getValues)) as Form.Rules[];
        }

        let rules: Form.Rules[] = cloneDeep(defRules) as Form.Rules[];
      ...
      function renderComponent() {
        ...
        const Comp = unref(driverHook?.useComponent(component)) || componentMap.get(component) as ReturnType<typeof defineComponent>;
      ...
      function renderItem() {
        ...
        if (component === 'Divider') {
          return (
            <WmqCol span={24}>
              <WmqDivider {...unref(getComponentsProps)}>{renderLabelHelpMessage()}</WmqDivider>
            </WmqCol>
          );
        } else {
          const getContent = () => {
      ...
      return () => {
        ...
        return (
          isIfShow && (
            <WmqCol {...realColProps} v-show={isShow}>
              {getContent()}
            </WmqCol>
          )
        );
