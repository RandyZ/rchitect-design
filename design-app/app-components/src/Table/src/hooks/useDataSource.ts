...
export function useDataSource(
  ...
  const dataSourceRef = ref<Recordable<any>[]>([]);
  const rawDataSourceRef = ref<Recordable<any>>({});
  ...
  function insertTableDataRecord(
    ...
    const _record = isObject(record) ? [record as Recordable<any>] : (record as Recordable<any>[]);
  ...
  async function fetch(opt?: FetchParams) {
    ...
    try {
      ...
      let params: Recordable<any> = merge(
        pageParams,
      ...
      let resultItems: Recordable<any>[] = isArrayResult ? res : get(res, listField);
  ...
  function getDataSource<T = Recordable<any>>() {
    return getDataSourceRef.value as T[];
  }

  function getRawDataSource<T = Recordable<any>>() {
