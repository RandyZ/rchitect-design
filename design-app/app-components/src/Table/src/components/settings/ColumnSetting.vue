<template>
  <WmqTooltip placement="top">
    ...
    >
      <template #reference>
        <WmqIconify icon="ant-design:setting-outlined" />
      </template>
...
<!--          {{ t('component.table.settingSelectColumnShow') }}-->
        ...
        <WmqButton size="small" type="link" @click="reset">
          {{ t('common.resetText') }}
        </WmqButton>
      </div>
      <ScrollContainer>
        <CheckboxGroup v-model:value="checkedList" @change="onChange" ref="columnListRef">
          <template v-for="item in plainOptions" :key="item.value">
            <div :class="`${prefixCls}__check-item`" v-if="!('ifShow' in item && !item.ifShow)">
              <WmqIconify icon="ant-design:drag-outlined" class="table-column-drag-icon" />
      ...
      function getColumns() {
        ...
        table.getColumns({ ignoreIndex: true, ignoreAction: true }).forEach((item) => {
          ret.push({
            ...
            value: (item.field || item.label) as string,
            ...item,
          });
        });
        return ret;
