...
export function useTree(treeDataRef: Ref<Tree.NodeData[]>, getFieldNames: ComputedRef<FieldNames>) {
  function getAllKeys(list?: Tree.NodeData[]) {
    const keys: string[] = [];
  ...
  function getEnabledKeys(list?: Tree.NodeData[]) {
    const keys: string[] = [];
    const treeData = list || unref(treeDataRef);
    const { key: keyField, children: childrenField } = unref(getFieldNames);
    if (!childrenField || !keyField) return keys;
  ...
  function getChildrenKeys(nodeKey: string | number, list?: Tree.NodeData[]) {
    const keys: KeyType[] = [];
    const treeData = list || unref(treeDataRef);
    const { key: keyField, children: childrenField } = unref(getFieldNames);
    if (!childrenField || !keyField) return keys;
  ...
  // Update node
  function updateNodeByKey(key: string, node: Tree.NodeData, list?: Tree.NodeData[]) {
    if (!key) return;
    const treeData = list || unref(treeDataRef);
    const { key: keyField, children: childrenField } = unref(getFieldNames);
  ...
  function filterByLevel(level = 1, list?: Tree.NodeData[], currentLevel = 1) {
    if (!level) {
      return [];
    }
  ...
  // Delete node
  function deleteNodeByKey(key: string, list?: Tree.NodeData[]) {
    if (!key) return;
    const treeData = list || unref(treeDataRef);
    const { key: keyField, children: childrenField } = unref(getFieldNames);
    if (!childrenField || !keyField) return;
